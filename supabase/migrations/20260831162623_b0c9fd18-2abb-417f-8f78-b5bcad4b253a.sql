...
BEGIN
  ...
  WHEN 'roles.assign' THEN
    ...
    PERFORM public.m00_emit('abox.m00.role.assigned.v1', 'role_assignment', v_id::text, COALESCE((p_payload->>'tenant_id')::uuid, p_tenant_id), p_payload, 'ROLE_ASSIGNED', p_user_id, NULL);
  ...
  WHEN 'tasks.transition' THEN
    ...
    PERFORM public.m00_emit(
      CASE WHEN upper(COALESCE(p_payload->>'status', '')) = 'COMPLETED' THEN 'abox.m00.task.completed.v1' ELSE 'abox.m00.task.status.changed.v1' END,
      'task', p_payload->>'task_id', p_tenant_id, p_payload, 'TASK_STATUS_CHANGED', p_user_id, NULL);
  ...
  WHEN 'exceptions.raise' THEN
    ...
    PERFORM public.m00_emit('abox.m00.exception.opened.v1', 'exception', v_id::text, p_tenant_id, p_payload, 'EXCEPTION_OPENED', p_user_id, NULL);
  ...
  WHEN 'events.publish' THEN
    v_id := public.m00_emit(COALESCE(p_payload->>'event_name', 'abox.m00.usage.recorded.v1'), COALESCE(p_payload->>'aggregate_type', 'platform'), COALESCE(p_payload->>'aggregate_id', 'manual'), p_tenant_id, COALESCE(p_payload->'data', '{}'::jsonb), 'EVENT_PUBLISHED', p_user_id, NULL);
...
REVOKE ALL ON FUNCTION public.m00_api(text, jsonb, uuid, uuid, boolean, text) FROM PUBLIC, anon, authenticated;
