...
export const M1_SCREENS: M1Screen[] = [
  {
    ...
    purpose:
      "Branded entry point for an agency's consumer marketplace. Establishes who is presenting the marketplace, what can be shopped, and the two ways in: guided (PlanAI) or self-directed browse.",
    components: [
      "Agency brand block: logo, marketplace name, licensed-entity line",
      "Hero with single primary value statement and ZIP capture",
      "Two entry cards — 'Help me choose' (PlanAI) and 'Browse plans myself'",
  ...
  },
  {
    ...
    user: "Consumer (D2C, unauthenticated)",
    purpose:
      "Consumer picks the product line to shop and the shopping path. This is the single screen that carries both the PlanAI entry and the Browse entry.",
    components: [
      ...
      "Path choice: guided PlanAI vs manual browse, each with plain-language description of what happens next",
  ...
  },
  {
    id: "UX-005",
    slug: "ux-005",
    name: "PlanAI Goals & Usage",
    group: "Consumer shopping",
    shell: "consumer",
    user: "Consumer (D2C) or agent in guided mode",
    purpose:
      "Guided-path intake. Captures priorities and expected usage so PlanAI can produce a ranked recommendation. This is the PlanAI entry proper.",
    components: [
      ...
      "Explanation panel: what PlanAI does and does not do",
    ],
    actions: [
      "Set priorities",
      "Set expected usage",
    ...
    compliance: [
      "PlanAI is a decision-support tool, not advice: a persistent disclosure states that results are informational and the consumer chooses the plan.",
      "The inputs used for ranking must be disclosed on the results screen; the consumer can change them at any time.",
      "Every PlanAI run is logged with its inputs, model/ruleset version and outputs for later governance review.",
    ],
    source: `${V4_WF} slide 5; ${V4_AI} (PlanAI ranking guardrails); ${V4_COMP}`,
    scope: "Protected Module 1 scope",
  ...
  },
  {
    ...
    purpose:
      "The core results surface. Carries both the PlanAI recommendation view and the manual browse view with filters, as two tabs over one result set.",
    components: [
      "Tabs: 'Recommended for you' (PlanAI ranked) and 'All plans' (manual browse)",
      "PlanAI panel on the recommended tab: ranked plans with fit reason, ranking-basis disclosure and 'why this plan' expander",
      ...
      "Sort control (premium, deductible, estimated total annual cost, PlanAI fit)",
  ...
  },
  {
    ...
    components: [
      ...
      "PlanAI fit explanation when arriving from the recommended tab",
      "Sticky action bar",
    ],
    ...
    compliance: [
      ...
      "Any PlanAI fit narrative is labeled as generated decision support, not a recommendation to purchase.",
    ],
  ...
  },
  {
    ...
    components: [
      "Left input rail: household, location, effective date, subsidy estimate inputs, PlanAI priorities",
  ...
  },
  {
    ...
    components: [
      ...
      "PlanAI enablement toggle and priority-set selection",
...
export const M1_REQUEST_MAP: { req: string; ids: string[] }[] = [
  ...
  { req: "PlanAI entry and Browse entry", ids: ["UX-002", "UX-005"] },
  ...
  { req: "PlanAI recommendation view", ids: ["UX-009", "UX-005"] },
