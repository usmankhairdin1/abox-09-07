...
export const CANONICAL_BOUNDARIES: BoundaryRecord[] = [
  {
    layer: "Foundations",
    belongsHere:
      "Raw colour, spacing, radius, shadow, type family, size and motion values declared in src/styles.css.",
    doesNotBelongHere:
      "Any decision about where a value is used. Foundations name values, not usage.",
    owner: "design-system",
  ...
  },
  {
    ...
    belongsHere:
      "Meaning attached to a foundation value: primary, muted, hairline, destructive, info, sage, warning, metal tiers.",
  ...
  },
  {
    ...
    belongsHere:
      "Reusable units with their own props: UI primitives and the ABox components (Button, Input, Select, StatusBadge, PageHeader, KpiCard, EmptyState, DataTable and peers).",
    doesNotBelongHere: "Screen assumptions, route data fetching, business rules.",
    owner: "design-system",
    evidence:
      "Phase 5 inventory: 49 UI primitives, 27 ABox modules, with measured consumer counts.",
    status: "CURRENT IMPLEMENTATION",
  },
  {
    layer: "Compounds",
    belongsHere:
      "Fixed compositions imported as one unit, such as the plan tile, the cart summary card and the metric card.",
  ...
  },
  {
    layer: "Patterns",
    belongsHere:
      "Recurring arrangements solving one screen problem, observed in at least two independent places.",
  ...
  },
  {
    ...
    belongsHere:
      "A pattern specialised for one experience: same anatomy, different composition, density or wording.",
  ...
  },
  {
    layer: "Shell systems",
    belongsHere:
      "InternalShell, MarketplaceShell and MemberShell: page frame, navigation model, account control per audience.",
    doesNotBelongHere:
      "A single shared navigation implementation. Three independent implementations exist by audience.",
    owner: "business",
  ...
  },
  {
    ...
    doesNotBelongHere:
      "Design-system ownership. A kit is owned by its module until adoption is explicitly approved.",
    owner: "route",
  ...
  },
  {
    ...
    doesNotBelongHere:
      "The design system. It may define the slot a brand value fills; it never owns the value.",
    owner: "runtime",
  ...
  },
  {
    ...
    doesNotBelongHere:
      "Any component or pattern record. The design system never encodes a business rule.",
    owner: "business",
