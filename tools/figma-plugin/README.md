...
(`current-app-verify`). `extract-current-app.mjs` derives its inventory from the current
production routes and the closed B9 identity/interaction manifest, then writes
`tokens-current-app.js`. The result is additive and uses only `07 Current App`.

The page contains deterministic module group frames with native editable desktop screen
compositions, plus 390px companions only where responsive source evidence exists. Existing
B4/B5 components, B6 patterns and B7 shells are live instances. Tables, plan cards, charts,
overlays and other structures without an approved foundation remain route-local Frames and
Text; they are not promoted to components. Existing B1–B3 styles are resolved by live IDs.

All 179 protected B9 identities are cross-referenced, and all 682 deterministic B9
interactions are mapped between the corresponding current-app frames. Runtime, dynamic,
...
unchanged IDs/signatures/coordinates, no overlaps or image fills, and a final B0–B10 verify
sweep. Until then the result is `REAL FIGMA NOT VERIFIED`.
