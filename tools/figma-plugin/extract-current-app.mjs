...
function sourceEvidence(screen) {
  if (!screen.sourceFile || !screen.sourceFile.startsWith("src/") || !readFileSync) return { text: "", labels: [] };
...
for (const module of moduleOrder) {
  ...
  members.forEach((screen, i) => {
    ...
    const x = 160 + col * 1600;
    const y = 256 + priorHeight;
    const h = screen.desktopHeight;
    placements.push({ key: screen.key, x, y, width: 1440, height: h });
    maxBottom = Math.max(maxBottom, y + h);
  });
  const width = 160 + Math.min(4, members.length) * 1440 + Math.max(0, Math.min(4, members.length) - 1) * 160 + 160;
...
for (const group of groups) for (let a = 0; a < group.placements.length; a += 1) for (let b = a + 1; b < group.placements.length; b += 1) {
  ...
  if (x.x < y.x + y.width && x.x + x.width > y.x && x.y < y.y + y.height && x.y + x.height > y.y) throw new Error("STOP: placement collision in " + group.name);
}
