...
// Deterministic zero-duplicate identity assertion — must pass before any manifest is written or any Figma write is attempted.
function assertNoDuplicates(label, values) {
  const seen = {}, collisions = [];
  for (const value of values) { if (seen[value]) { if (collisions.indexOf(value) === -1) collisions.push(value); } seen[value] = true; }
  if (collisions.length) throw new Error("STOP: duplicate current-app " + label + " — " + collisions.map((v) => '"' + v + '"').join(", "));
}
assertNoDuplicates("stable key", screens.map((screen) => screen.key));
assertNoDuplicates("screen name", screens.map((screen) => screen.name));
assertNoDuplicates("frame name", screens.flatMap((screen) => screen.viewports.includes(390) ? [screen.name, "mobile/" + screen.name] : [screen.name]));
assertNoDuplicates("group name", groups.map((group) => group.name));

// Regression check for the Phase 59 real-Figma failure: governed IDs shared across routes/states must not collapse into one name.
const regressionKeys = ["current:route:/app/agency", "current:route:/app/agency/entities", "current:route:/plans", "current:state:plans:filters", "current:state:plans:edit-quote"];
const regressionScreens = regressionKeys.map((key) => {
  const screen = screens.find((candidate) => candidate.key === key);
  if (!screen) throw new Error('STOP: regression screen identity missing — "' + key + '".');
  return screen;
});
assertNoDuplicates("regression screen name", regressionScreens.map((screen) => screen.name));
